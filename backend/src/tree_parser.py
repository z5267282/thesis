...
def iterate_until_and_parse_first_line(
        code: Iterator[tuple[int, str]], stack: Stack[BodyBlock]) -> tuple[BodyBlock, int]:
...
def parse_line(line: str, line_no: int,
               indent_level: int) -> CodeBlock | BodyBlock:
...
def parse_unindented_block(
    ...
    if isinstance(block, BodyBlock) and is_branch(block):
        # we can cast because we know at runtime that the top must be an
        # IfBlock if we receive one of its branches
        add_branch(
            cast(
                ElifBlock | ElseBlock, block), prev, cast(
                IfBlock, top), stack)
    else:
